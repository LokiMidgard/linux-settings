...
if curl -fsSL --head --insecure --request GET https://step.fritz.box/certs/root_ca.crt | grep "HTTP/2 200" > /dev/null; then
    ...
    if ! cmp -s /usr/local/share/ca-certificates/personal/new_root_ca.crt /usr/local/share/ca-certificates/personal/root_ca.crt; then
        echo "Neues Zertifikat unterscheidet sich von /usr/local/share/ca-certificates/personal/root_ca.crt. Zertifikat wird aktualisiert."
        # update the certificate
        sudo cp /usr/local/share/ca-certificates/personal/new_root_ca.crt /usr/local/share/ca-certificates/personal/root_ca.crt
